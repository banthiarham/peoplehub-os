...
import { Calculator, FileClock, GitCompareArrows, Landmark, LockKeyhole, ReceiptText } from 'lucide-react';
...
export default function TaxPage() {
  ...
  const { data: regimes } = useQuery({
    ...
    queryFn: () =>
      api.get('/tax/regimes', { params: { tenantId, taxYearId: activeYear?.id } }).then((r) => r.data as RegimeConfig[]),
  ...
  const { data: slabs, isLoading: slabsLoading } = useQuery({
    ...
    queryFn: () => api.get('/tax/slabs', { params: { tenantId, taxYearId: activeYear?.id } }).then((r) => r.data as TaxSlab[]),
    enabled: Boolean(tenantId && activeYear?.id),
  });

  const below60New = slabs?.filter((slab) => slab.regime === 'NEW' && slab.ageCategory === 'BELOW_60') ?? [];
  const below60Old = slabs?.filter((slab) => slab.regime === 'OLD' && slab.ageCategory === 'BELOW_60') ?? [];
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Financial year" value={activeYear?.financialYear ?? 'Not set'} icon={FileClock}>
          <p className="text-xs text-ink-muted">AY {activeYear?.assessmentYear ?? '-'}</p>
        </StatCard>
        <StatCard label="Default regime" value={regimes?.find((r) => r.isDefault)?.regime ?? 'NEW'} icon={Landmark}>
          <p className="text-xs text-ink-muted">Section 115BAC default, old regime opt-out supported</p>
        </StatCard>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.1fr_1.9fr]">
        <Card>
          <CardHeader>
            ...
            <CardDescription>PRD-required controls for tax-year governance and payroll readiness.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="rounded-lg border border-line bg-slate-50 p-4">
              <p className="text-sm font-semibold text-ink">{activeYear?.country ?? 'IN'} tax year</p>
              <div className="mt-3 grid gap-3 text-sm">
                ...
                <Row label="Status" value={<Badge variant={activeYear?.isActive ? 'success' : 'outline'}>{activeYear?.isActive ? 'Active' : 'Draft'}</Badge>} />
                <Row label="Default year" value={<Badge variant={activeYear?.isDefault ? 'success' : 'outline'}>{activeYear?.isDefault ? 'Yes' : 'No'}</Badge>} />
              </div>
            </div>

            <div className="space-y-2">
              {(regimes ?? []).map((regime) => (
                <div key={regime.id} className="flex items-center justify-between rounded-lg border border-line px-3 py-2.5">
                  <div>
                    <p className="text-sm font-semibold">{regime.name}</p>
                    <p className="text-xs text-ink-muted">Employees can select: {regime.employeeCanSelect ? 'Yes' : 'No'}</p>
                  </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>AY {activeYear?.assessmentYear ?? '-'} slab configuration from the database.</CardDescription>
          </CardHeader>
          <Table>
            <THead>
              <TR>
            ...
            <TBody>
              ...
              {[...below60New, ...below60Old].map((slab) => (
                <TR key={slab.id}>
                  <TD>
                    <Badge variant={slab.regime === 'NEW' ? 'success' : 'info'}>{slab.regime}</Badge>
                  </TD>
                  <TD className="font-medium">
                    {formatINR(slab.minIncome)} - {slab.maxIncome ? formatINR(slab.maxIncome) : 'Above'}
                  </TD>
          ...
          return (
            <Card key={age}>
              ...
              <CardContent>
                ...
                <p className="mt-1 text-sm text-ink-muted">Published slab rows across old and new regimes</p>
              </CardContent>
            </Card>
          );
        })}
